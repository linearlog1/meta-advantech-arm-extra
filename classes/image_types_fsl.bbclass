...
# Update partition volume id
UPDATEDD_VOLUME_ID ?= "Update ${MACHINE}"

#Set alighment to 256MB [in KiB]
ROOTFS_UPDATE_SIZE ?= "262144"
...
#
# Generate the update image with the FAT32 filesystem type
_generate_update_image() {
	...
	# mkdosfs will sometimes use FAT16 when it is not appropriate,
        ...
        mkfs.vfat -n "${UPDATEDD_VOLUME_ID}" -S 512 ${FATSIZE} -C ${WORKDIR}/update.img $BOOT_BLOCKS
}
...
#                                                     <--------->
#            4MiB               8MiB           SDIMG_ROOTFS                    4MiB                              256MiB
# <-----------------------> <----------> <----------------------> <------------------------------> <------------------------------------>
#  ------------------------ ------------ ------------------------ -------------------------------  -------------------------------------
# | IMAGE_ROOTFS_ALIGNMENT | BOOT_SPACE | ROOTFS_SIZE            |     IMAGE_ROOTFS_ALIGNMENT    |         ROOTFS_UPDATE_SPACE
#  ------------------------ ------------ ------------------------ -------------------------------  -------------------------------------
# ^                        ^            ^                        ^                               ^                                     ^
# |                        |            |                        |                               |                                     |
# 0                      4096     4MiB +  8MiB       4MiB +  8Mib + SDIMG_ROOTFS   4MiB +  8MiB + SDIMG_ROOTFS + 4MiB          4MiB +  8MiB + SDIMG_ROOTFS + 4MiB + 256MiB
generate_imx_sdcard () {
	# Create partition table
	parted -s ${SDCARD} mklabel msdos
	parted -s ${SDCARD} unit KiB mkpart primary fat32 ${IMAGE_ROOTFS_ALIGNMENT} $(expr ${IMAGE_ROOTFS_ALIGNMENT} \+ ${BOOT_SPACE_ALIGNED})
	parted -s ${SDCARD} unit KiB mkpart primary $(expr  ${IMAGE_ROOTFS_ALIGNMENT} \+ ${BOOT_SPACE_ALIGNED}) $(expr ${IMAGE_ROOTFS_ALIGNMENT} \+ ${BOOT_SPACE_ALIGNED} \+ $ROOTFS_SIZE)
	parted -s ${SDCARD} unit KiB mkpart primary fat32 $(expr ${IMAGE_ROOTFS_ALIGNMENT} \+ ${BOOT_SPACE_ALIGNED} \+ $ROOTFS_SIZE) $(expr ${IMAGE_ROOTFS_ALIGNMENT} \+ ${BOOT_SPACE_ALIGNED} \+ $ROOTFS_SIZE \+ ${ROOTFS_UPDATE_SIZE})
	parted ${SDCARD} print
	...
	dd if=${WORKDIR}/update.img of=${SDCARD} conv=notrunc,fsync seek=1 bs=$(expr ${BOOT_SPACE_ALIGNED} \* 1024 + ${IMAGE_ROOTFS_ALIGNMENT} \* 1024 + $ROOTFS_SIZE \* 1024)
}
...
IMAGE_CMD_sdcard () {
	...
	SDCARD_SIZE=$(expr ${IMAGE_ROOTFS_ALIGNMENT} + ${BOOT_SPACE_ALIGNED} + $ROOTFS_SIZE + ${IMAGE_ROOTFS_ALIGNMENT} + ${ROOTFS_UPDATE_SIZE})
